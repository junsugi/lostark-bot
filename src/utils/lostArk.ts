...
export async function getUserInfo(userName: string){
    ...
    return new Promise((resolve, reject) => {
        getHtml(`https://lostark.game.onstove.com/Profile/Character/${encodeURI(userName)}#profile-collection`).then((html: any) => {
...
export function getEventMessageEmbed(){
    console.log("call getEventMessageEmbed !!");
    return new Promise((resolve, reject) => {
        getHtml("https://lostark.game.onstove.com/News/Event/Now").then((html: any) => {
            const $ = cheerio.load(html.data);
            let event: any = {};
            // 링크
            let temp: any = [];
