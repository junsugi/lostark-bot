...
export async function getUserInfo(userName: string){
    const html: any = await getHtml(`https://lostark.game.onstove.com/Profile/Character/${encodeURI(userName)}#profile-collection`);
    const $ = cheerio.load(html.data);
    let json: any = {};
    // 수집형 포인트
    $("div.collection-status").addClass("active");
    // 닉네임
...
export async function getEventMessageEmbed(){
    const html: any = await getHtml("https://lostark.game.onstove.com/News/Event/Now");
    const $ = cheerio.load(html.data);
    let event: any = {};
    // 링크
    let temp: any = [];
    ...
    temp = [];
    let temp2: any = [];
    $("div.list.list--event > ul > li > a > div.list__thumb > img").each(function(index){
    ...
    // 이벤트 기간
    temp = [];
    $("div.list__term").each(function(index){
